  ...
  try {
    ...
    if (project_config.cloud_provider === "aws") {
      ...
      if (fs.existsSync(terraformStateFile)) {
        ...
        const lockId = execSync(lockIdCommand, {
          cwd: projectPath,
          stdio: "pipe",
        })
          .toString()
        ...
        throw new Error(
          `Terraform state file not found: ${terraformStateFile}`
        );
      }
    } 
  } catch (error) {
