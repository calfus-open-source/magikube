...
export default abstract class TerraformProject {
  static async getProject(
    command: BaseCommand,
  ): Promise<(BaseProject & CloudProject) | null> {
    const config = SystemConfig.getInstance().getConfig();
    if (config.cloud_provider === 'aws') {
      if (config.cluster_type === 'eks-fargate')
        return new EKSFargateProject(command, config) as BaseProject &
          CloudProject;
      ...
      if (config.cluster_type === 'eks-nodegroup')
        return new EKSNodeGrpClusterProject(command, config) as BaseProject &
          CloudProject;

      command.error(
        `Cloud provider '${config.cloud_provider}' and cluster type '${config.cluster_type}' not supported`,
      );
    } else if (config.cloud_provider === 'azure') {
      if (config.cluster_type === 'aks')
        return new AzureAKSProject(command, config) as BaseProject &
          CloudProject;
