...
export default class AzureTerraformBackend {
  ...
  static async create(
    ...
    const storageAccountName = `${projectId.replace(/-/g, '')}tfstate`.toLowerCase();
  ...
  static async createResourceGroup(
    ...
    try {
      AppLogger.info(`Checking if resource group ${resourceGroupName} exists...`, true);
      
      // Check if resource group exists
      try {
        execSync(`az group show --name "${resourceGroupName}"`, { stdio: 'pipe' });
        AppLogger.info(`Resource group ${resourceGroupName} already exists.`, true);
        return true;
      } catch (error) {
        // Resource group doesn't exist, create it
        AppLogger.info(`Resource group ${resourceGroupName} does not exist. Creating...`, true);
        ...
        AppLogger.info(`Resource group ${resourceGroupName} created successfully`, true);
        AppLogger.debug(result, true);
        return true;
      }
    } catch (err) {
      AppLogger.error(`Error creating resource group ${resourceGroupName}: ${err}`, true);
      return false;
    }
  }
  ...
  static async createStorageAccount(
    ...
    try {
      AppLogger.info(`Checking if storage account ${storageAccountName} exists...`, true);
      
      // Check if storage account exists
      try {
        execSync(`az storage account show --name "${storageAccountName}" --resource-group "${resourceGroupName}"`, { stdio: 'pipe' });
        AppLogger.info(`Storage account ${storageAccountName} already exists.`, true);
        return true;
      } catch (error) {
        // Storage account doesn't exist, create it
        AppLogger.info(`Storage account ${storageAccountName} does not exist. Creating...`, true);
        ...
        AppLogger.info(`Storage account ${storageAccountName} created successfully`, true);
        AppLogger.debug(result, true);
        return true;
      }
    } catch (err) {
      AppLogger.error(`Error creating storage account ${storageAccountName}: ${err}`, true);
      return false;
    }
  }
  ...
  static async createBlobContainer(
    ...
    try {
      AppLogger.info(`Retrieving storage account key for ${storageAccountName}...`, true);
      ...
      AppLogger.info(`Checking if container ${containerName} exists...`, true);
      
      // Check if container exists
      try {
        execSync(`az storage container show --name "${containerName}" --account-name "${storageAccountName}" --account-key "${storageKey}"`, { stdio: 'pipe' });
        AppLogger.info(`Container ${containerName} already exists.`, true);
        return true;
      } catch (error) {
        // Container doesn't exist, create it
        AppLogger.info(`Container ${containerName} does not exist. Creating...`, true);
        ...
        AppLogger.info(`Container ${containerName} created successfully`, true);
        AppLogger.debug(result, true);
        return true;
      }
    } catch (err) {
      AppLogger.error(`Error creating blob container ${containerName}: ${err}`, true);
      return false;
    }
  }
  ...
  static async deleteResourceGroup(
    ...
    try {
      ...
      try {
        ...
        const deleteCommand = `az group delete --name "${resourceGroupName}" --yes --no-wait`;
        execSync(deleteCommand, { encoding: 'utf8' });
        
        AppLogger.info(`Resource group ${resourceGroupName} deletion initiated`, true);
        return true;
      } catch (error) {
  ...
  static getStorageAccountKey(storageAccountName: string, resourceGroupName: string): string | null {
    try {
      const keyCommand = `az storage account keys list --resource-group "${resourceGroupName}" --account-name "${storageAccountName}" --query '[0].value' --output tsv`;
      return execSync(keyCommand, { encoding: 'utf8' }).trim();
    } catch (error) {
      AppLogger.error(`Failed to retrieve storage account key for ${storageAccountName}`, true);
      return null;
    }
  }
