...
export default class CreateApplication extends BaseProject {
  ...
  async setupAuthenticationService(projectConfig: any) {
    ...
    try {
      ...
      const keyCloakBaseFiles = [
        'app.controller.ts',
        'app.module.ts',
        'app.service.ts',
        'main.ts',
      ];
      const keyCloakHealthFiles = [
      ...
      for (const file of githubActionFIles) {
        await this.createFile(
          `${file}`,
          `${copyFilePath}/dist/keycloak-auth-service/${file}.liquid`,
  ...
  async setupKeyCloak(projectConfig: any) {
    ...
    try {
      ...
      for (const file of githubActionFIles) {
        await this.createFile(
          `${file}`,
          `${copyFilePath}/dist/keycloak/${file}.liquid`,
          `${createFilePath}/.github/workflows`,
          true,
        );
      }
      ...
      for (const file of themeSetupFiles) {
        await this.createFile(
          file,
          `${copyFilePath}/dist/keycloak/${file}.liquid`,
          `${createFilePath}/themes/magikube/login`,
          true,
        );
      }
  ...
  createNodeExpressApp = async (configObject: any) => {
    ...
    try {
      ...
      await this.createFile(
        'app.ts',
        `${copyFilePath}/dist/express/app.ts.liquid`,
        `${createFilePath}/src`,
        true,
      );
      ...
      for (const file of githubActionFIles) {
        await this.createFile(
          `${file}`,
          `${copyFilePath}/dist/express/${file}.liquid`,
          `${createFilePath}/.github/workflows`,
          true,
        );
      }
      ...
      await this.createFile(
        'AppConfig.ts',
        `${copyFilePath}/dist/express/AppConfig.ts.liquid`,
        `${createFilePath}/src/config`,
        true,
      );
      await this.createFile(
        'auth-guard.service.ts',
        `${copyFilePath}/dist/express/auth-guard.service.ts.liquid`,
        `${createFilePath}/src/middlewares`,
        true,
      );
      await this.createFile(
        'auth-guard.ts',
        `${copyFilePath}/dist/express/auth-guard.ts.liquid`,
        `${createFilePath}/src/middlewares`,
        true,
      );
      await this.createFile(
        'index.ts',
        `${copyFilePath}/dist/express/index.ts.liquid`,
        `${createFilePath}/src/routes`,
        true,
      );
      await this.createFile(
        'protected.ts',
  ...
  createNextApp = async (configObject: any) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        await this.createFile(
          file,
          `${copyFilePath}/dist/next/${file}.liquid`,
          `${createFilePath}`,
          true,
        );
      }
      ...
      await this.createFile(
        `page.tsx`,
        `${copyFilePath}/dist/next/callback.tsx.liquid`,
        `${createFilePath}/app/callback`,
        true,
      );
      for (const file of githubActionFiles) {
        await this.createFile(
  ...
  createReactApp = async (configObject: any) => {
    ...
    try {
      ...
      await this.createFile(
        'index.html',
        `${copyFilePath}/dist/react/index.html.liquid`,
        `${createFilePath}/public`,
        true,
      );
      ...
      for (const file of reactAppFile) {
        await this.createFile(
          file,
          `${copyFilePath}/dist/react/${file}.liquid`,
          `${createFilePath}/src`,
          true,
        );
      }
      const reactAuthFiles = [
        'AuthGuard.tsx',
        'AuthenticationProvider.tsx',
        'Callback.tsx',
      ];
      for (const file of reactAuthFiles) {
        await this.createFile(
          file,
          `${copyFilePath}/dist/react/${file}.liquid`,
          `${createFilePath}/src/components`,
          true,
        );
      }
      const reactCommonFiles = [
        'package.json',
        'tsconfig.json',
        'Dockerfile',
        'nginx.conf',
  ...
  async setupGitops(projectConfig: any) {
    ...
    try {
      ...
      for (const file of gitopsFiles) {
        await this.createFile(
          file,
          `${filePath}/dist/gitops/auth-gitops/${file}.liquid`,
          `gitops/${projectName}-${environment}/keycloak-auth-service`,
          true,
        );
      }
      ...
      for (const file of gitopsKeycloakFiles) {
        await this.createFile(
          file,
          `${filePath}/dist/gitops/keycloak-gitops/${file}.liquid`,
          `gitops/${projectName}-${environment}/keycloak`,
          true,
        );
      }
      for (const file of gitopsFiles) {
        await this.createFile(
          file,
          `${filePath}/dist/gitops/express-gitops/${file}.liquid`,
          `gitops/${projectName}-${environment}/express`,
          true,
        );
      }
      if (frontend_app_type == 'react') {
        for (const file of gitopsFiles) {
          await this.createFile(
            file,
            `${filePath}/dist/gitops/react-gitops/${file}.liquid`,
            `gitops/${projectName}-${environment}/react`,
            true,
          );
        }
        await this.createFile(
          'react.yml',
          `${filePath}/dist/gitops/common-gitops-files/react.yml.liquid`,
          `gitops/${projectName}-${environment}`,
          true,
        );
      }
      if (frontend_app_type == 'next') {
        for (const file of gitopsFiles) {
          await this.createFile(
            file,
            `${filePath}/dist/gitops/next-gitops/${file}.liquid`,
            `gitops/${projectName}-${environment}/next/`,
            true,
          );
        }
        await this.createFile(
          'next.yml',
          `${filePath}/dist/gitops/common-gitops-files/next.yml.liquid`,
          `gitops/${projectName}-${environment}`,
          true,
        );
      }
      for (const file of commonGitopsFiles) {
        await this.createFile(
          `${file}`,
          `${filePath}/dist/gitops/common-gitops-files/${file}.liquid`,
          `gitops/${projectName}-${environment}`,
          true,
        );
      }
      ...
      updateStatusFile(projectName, 'gitops', 'success');
      fs.rmdirSync(`${filePath}/${projectName}/${appName}`, {
        recursive: true,
      });
      process.exit(1);
    }
    ...
    try {
      ...
      for (const appName of appNames) {
        ...
        if (url) {
          ...
          try {
            await executeCommandWithRetry(command, { stdio: 'pipe' }, 3);
