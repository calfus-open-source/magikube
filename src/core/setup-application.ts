...
export default class CreateApplication extends BaseProject {
  ...
  async setupAuthenticationService(projectConfig: any) {
    ...
    try {
      ...
      const commonFiles = ['package.json', 'tsconfig.json', 'Dockerfile', 'tsconfig.build.json', 'nest-cli.json', 'README.md'];
  ...
  createGenAIApp = async (configObject: any) => {
    const projectConfig = SystemConfig.getInstance().getConfig();
    const { appName, projectName } = configObject;
    const genAIAppName = this.config.command === "create" ? projectConfig.service_name : appName;
    let copyFilePath;
    let createFilePath;
    let applicationPath;
    if (this.config.command === "create") {
