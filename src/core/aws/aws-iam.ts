...
export default class AWSPolicies {
  static async create(
    ...
    const account = await AWSAccount.getAccountId(
      accessKeyId,
      secretAccessKey,
      region,
    );
    ...
    const addUserToGroup = async (userName: string, groupName: string) => {
      try {
        ...
        AppLogger.info(
          `User ${userName} added to group ${groupName}, ${JSON.stringify(data)}`,
        );
      } catch (err) {
        AppLogger.error(
          `Error adding user ${userName} to group ${groupName}, ${err}`,
          true,
        );
      }
    };
    ...
    const attachGroupPolicy = async (groupName: string, policyArn: string) => {
      try {
        ...
        AppLogger.info(
          `Policy ${policyArn} attached to group ${groupName} ${JSON.stringify(data)}`,
        );
      } catch (err) {
        AppLogger.error(
          `Error attaching policy ${policyArn} to group ${groupName}, ${err}`,
          true,
        );
      }
    };
    ...
    if (
      ...
      const files = fs.readdirSync(
        join(folderpath, 'dist/templates/aws/policies'),
      );
      try {
        for (const file of files) {
          const policyName = `${projectName}-${file.split('.')[0]}`;
          AppLogger.info(
            `Creating policy, user, group, and adding the user to the group: ${policyName}`,
            true,
          );
          const policyDocument = await project.generateContent(
            `${policyPath}/${file}`,
          );
          await createPolicy(policyName, policyDocument);
          await createGroup(policyName);
          await attachGroupPolicy(
            policyName,
            `arn:aws:iam::${account}:policy/${policyName}`,
          );
  ...
  static async delete(
    ...
    const files = fs.readdirSync(
      join(`${process.cwd()}/dist/templates/aws/policies`),
    );
    ...
    for (const file of files) {
      ...
      AppLogger.info(
        `Deleting policy, user, group and adding the user to the group: ${policyName}`,
        true,
      );
      ...
      try {
        ...
        AppLogger.info(
          `Policy ${policyName} detached from group, ${JSON.stringify(data)}`,
        );
      } catch (err) {
        AppLogger.error(
          `Error detaching policy ${policyName} from group, ${err}`,
          true,
        );
      }
      ...
      try {
        ...
        AppLogger.info(
          `User ${policyName} removed from group, ${JSON.stringify(data)}`,
        );
      } catch (err) {
        AppLogger.error(
          `Error removing user ${policyName} from group, ${err}`,
          true,
        );
      }
      ...
      if (
        ...
        try {
          ...
          AppLogger.error(
            `Error deleting Git credentials for user ${policyName}: ${err}`,
            true,
          );
        }
      }
