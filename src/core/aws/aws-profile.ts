...
export default class AWSProfile {
  static getProfiles(): any {
    ...
    if (fs.existsSync(credentialsFilePath)) {
      ...
      while ((match = profileRegex.exec(credentialsFileContent))) {
        ...
        const profileContent = credentialsFileContent.substring(
          match.index + match[0].length,
        );
        const awsAccessKey =
          /aws_access_key_id = (\S+)/.exec(profileContent)?.[1] || '';
        const awsSecretAccessKey =
          /aws_secret_access_key = (\S+)/.exec(profileContent)?.[1] || '';
  ...
  static addProfile(
    ...
    AppLogger.debug(
      `Adding AWS profile ${profileName} to ${credentialsFilePath}`,
    );
