...
export default class AWSProject extends BaseProject {
  ...
  async createVpc(): Promise<void> {
    this.createFile(
      "main.tf",
      `${process.cwd()}/dist/templates/aws/modules/vpc/main.tf.liquid`,
      "/infrastructure/modules/vpc",
      true
    );
    this.createFile(
      "variables.tf",
      `${process.cwd()}/dist/templates/aws/modules/vpc/variables.tf.liquid`,
      "/infrastructure/modules/vpc",
      true
    );
  }
  ...
  async createRds(): Promise<void> {
    this.createFile(
      "main.tf",
      `${process.cwd()}/dist/templates/aws/modules/rds/main.tf.liquid`,
      "/infrastructure/modules/rds",
      true
    );
    this.createFile(
      "variables.tf",
      `${process.cwd()}/dist/templates/aws/modules/rds/variables.tf.liquid`,
      "/infrastructure/modules/rds",
      true
    );
  }
  ...
  async createACM(): Promise<void> {
    this.createFile(
      "main.tf",
      `${process.cwd()}/dist/templates/aws/modules/acm/main.tf.liquid`,
      "/infrastructure/modules/acm",
      true
    );
    this.createFile(
      "variables.tf",
      `${process.cwd()}/dist/templates/aws/modules/acm/variables.tf.liquid`,
      "/infrastructure/modules/acm",
      true
    );
  }
  ...
  async runTerraformApply(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (
          (module && projectConfig.command === "new_sub") ||
          (module && projectConfig.command === "new")
        ) {
  ...
  // async runTerraformDestroyTemplate(projectPath: string, modules:any,  varFile?: string): Promise<void> {
  // AppLogger.info(`Running terraform destroy... in ${projectPath}`, true);
  //  let awsStatus = false;
  //  if (this.config.cloud_provider === "aws") {
  //    awsStatus = true;
  ...
  async runTerraformDestroyTemplate(
    ...
    console.log(project_config.appliedModules, "<<<<<project_config.appliedModules");
    // Loop through each module and destroy it
    for (const module of project_config.appliedModules) {
      AppLogger.info(`Destroying module: ${module}`, true);

      let command = `terraform destroy -auto-approve -target=${module}`;
      if (varFile) {
        command += ` -var-file=${varFile}`;
      }

      // Execute the terraform destroy command for the current module
      await executeCommandWithRetry(
        command,
        { cwd: projectPath, stdio: "inherit" },
        3
      ...
      AppLogger.info(`Terraform destroy completed for module: ${module}`, true);
    }

    AppLogger.info(
      "Terraform destroy for all modules completed successfully.",
      true
    );

    // Cleanup AWS resources if cloud provider is AWS
    if (awsStatus) {
      const status = await AWSPolicies.delete(
        this,
        this.config.aws_region,
      ...
      if (status) {
        ...
        AppLogger.info("AWS Terraform backend deleted successfully.", true);
      }
    }
  }
