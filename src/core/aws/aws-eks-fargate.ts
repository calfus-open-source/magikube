...
export default class EKSFargateProject extends AWSProject {
  ...
  async createMainFile(): Promise<void> {
    ...
    this.createFile(
      'main.tf',
      `${path}/dist/templates/aws/eks-fargate/main.tf.liquid`,
      `/infrastructure`,
      true,
    );
    this.createFile(
      'terraform.tfvars',
      `${path}/dist/templates/aws/eks-fargate/terraform.tfvars.liquid`,
      `/infrastructure`,
      true,
    );
    this.createFile(
      'variables.tf',
      `${path}/dist/templates/aws/eks-fargate/variables.tf.liquid`,
      '/infrastructure',
      true,
    );
    this.createFile(
      `${this.config.environment}-config.tfvars`,
      `${path}/dist/templates/aws/eks-fargate/backend-config.tfvars.liquid`,
      '/infrastructure',
      true,
    );
