...
function validateUserInput(input: string): void {
    ...
    if (pattern.test(input)) {
        ...
        throw new Error(`\n \n  ERROR: Project Name "${input}" is invalid. It must start with an alphabet, must include only lowercase alphabets, numbers, or underscores, length of string must be [3-8] and must not end with an underscore. \n \n`);
    }
}
