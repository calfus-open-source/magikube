...
import {
  aws_modules as modules,
  playbooks,
} from '../../src/core/constants/constants.js';
import {
  handleEKSandAKS,
  handleK8s,
} from '../../src/core/utils/terraformHandlers-utils.js';
...
describe('handleEKS', () => {
  ...
  it('should initialize terraform and apply all modules successfully', async () => {
    ...
    expect(mockTerraform.runTerraformApply).toHaveBeenCalledTimes(
      modules.length,
    );
...
describe('handleK8s', () => {
  ...
  it('should run terraform init/apply and ansible playbooks', async () => {
    ...
    expect(mockTerraform.runAnsiblePlaybook).toHaveBeenCalledTimes(
      playbooks.length,
    );
  ...
  it('should log error if AWS export fails', async () => {
    ...
    expect(AppLogger.error).toHaveBeenCalledWith(
      'AWS export command NOT executed',
      true,
    );
  });
});
