...
const mockedExecuteCommand = executeCommandWithRetry as jest.MockedFunction<
...
describe('runTerraformUnlockCommands', () => {
  ...
  const terraformStateFile = path.join(
    infrastructurePath,
    '.terraform',
    'terraform.tfstate',
  );
  ...
  it('throws error if terraform state file does not exist', async () => {
    ...
    await expect(
      runTerraformUnlockCommands(projectPath, project_config),
    ).rejects.toThrow(/Terraform state file not found/);
