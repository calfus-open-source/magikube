...
beforeEach(() => {
  ...
  createApplicationInstance = new CreateApplication(
    MockCommand,
    mockBaseProjectConfig,
  );
...
describe('CreateApplication.setupAuthenticationService', () => {
  test('should successfully create auth-service files and run npm install (new command)', async () => {
    ...
    const result = await createApplicationInstance.setupAuthenticationService(
      mockBaseProjectConfig,
    );
  ...
  test('should handle failure during auth-service setup', async () => {
    ...
    (executeCommandWithRetry as jest.Mock).mockRejectedValue(
      new Error('npm failed'),
    );

    await createApplicationInstance.setupAuthenticationService(
      mockBaseProjectConfig,
    );
    ...
    expect(fs.rmdirSync).toHaveBeenCalledWith(applicationPath, {
      recursive: true,
    });
...
describe('CreateApplication.setupKeyCloak', () => {
  ...
  test('should handle failure during keycloak setup', async () => {
    ...
    const { applicationPath } = getExpectedPaths(expectedAppName);
    createApplicationInstance.createFile.mockRejectedValue(
...
describe('CreateApplication App Creation', () => {
  ...
  const testAppCreation = async (
    ...
    await method({
      appName: mockBaseProjectConfig[appNameKey],
      projectName: mockBaseProjectConfig.project_name,
    });
    if (appName !== 'genai-app') {
      // GenAI does not run npm install
      expect(executeCommandWithRetry).toHaveBeenCalledWith(
        'npm install',
    ...
    expect(updateStatusFile).toHaveBeenCalledWith(
      mockBaseProjectConfig.project_name,
      appName,
      expect.any(String),
    );
  ...
  const testAppCreationFailure = async (
    methodName: string,
    appNameKey: string,
    appName: string,
  ) => {
  ...
  test('should handle node-express app creation failure', async () => {
    await testAppCreationFailure(
      'createNodeExpressApp',
      'node_app_name',
      'node-app',
    );
  });
  ...
  test('should successfully create GenAI app', async () => {
    ...
    await method({
      genAI_app_name: mockBaseProjectConfig.genAI_app_name,
      projectName: mockBaseProjectConfig.project_name,
    });
    expect(updateStatusFile).toHaveBeenCalledWith(
      mockBaseProjectConfig.project_name,
  ...
  test('should handle GenAI app creation failure', async () => {
    ...
    await createApplicationInstance.createGenAIApp({
      genAI_app_name: 'genai-app',
...
describe('CreateApplication.setupGitops', () => {
  ...
  test('should successfully create all gitops files with react frontend', async () => {
    ...
    const result = await createApplicationInstance.setupGitops(
      mockBaseProjectConfig,
    );
  ...
  test('should handle failure during gitops setup', async () => {
    ...
    expect(fs.rmdirSync).toHaveBeenCalledWith(expectedRmdirPath, {
      recursive: true,
    });
    expect(mockExit).toHaveBeenCalledWith(1);
  });
...
describe('CreateApplication.handleAppCreation', () => {
  ...
  beforeEach(() => {
    ...
    createApplicationInstance.createReactApp = jest
      .fn()
      .mockResolvedValue(true);
    // Ensure the appTypeMap references the mocked function (constructor set earlier)
    if (
      createApplicationInstance.appTypeMap &&
      createApplicationInstance.appTypeMap['react']
    ) {
      createApplicationInstance.appTypeMap['react'].createAppFunction =
        createApplicationInstance.createReactApp;
    }
  });

  test('should call createAppFunction and then pushCode on success', async () => {
    await createApplicationInstance.handleAppCreation(
      'react',
      mockConfigObject,
      mockBaseProjectConfig,
    );
    ...
    expect(mockConfigObject.common.appName).toBe(
      mockBaseProjectConfig.react_app_name,
    );
    expect(mockConfigObject.common.appType).toBe(
      mockBaseProjectConfig.frontend_app_type,
    );
  });
  ...
  test('should handle error if pushCode fails', async () => {
    (ManageRepository.pushCode as jest.Mock).mockResolvedValue(
      'Repo Setup Error',
    );
    const mockConfigObject = {
      projectId: 1,
      common: { appName: '', appType: '' },
    };
    ...
    expect(AppLogger.error).toHaveBeenCalledWith(
      'Error occured while setting up the repository',
      true,
    );
    expect(mockExit).toHaveBeenCalledWith(1);
  });
...
describe('CreateApplication.destroyApp', () => {
  ...
  test('should delete both frontend and backend repos and local folders (with org)', async () => {
    ...
    await createApplicationInstance.destroyApp(
      mockUser,
      mockAuth,
      mockOrg,
      mockFrontend,
  ...
  test('should log error if repository deletion fails', async () => {
    (fs.existsSync as jest.Mock).mockReturnValue(true);
    (executeCommandWithRetry as jest.Mock).mockRejectedValueOnce(
      'Repo delete failed',
    );
    ...
    expect(AppLogger.error).toHaveBeenCalledWith(
      expect.stringContaining('Repo delete failed'),
      true,
    );
  ...
  test('should log error if GitHub username is missing', async () => {
    ...
    expect(AppLogger.error).toHaveBeenCalledWith(
      'Error: Missing GitHub username or organization name',
      true,
    );
    expect(executeCommandWithRetry).not.toHaveBeenCalled();
  });
